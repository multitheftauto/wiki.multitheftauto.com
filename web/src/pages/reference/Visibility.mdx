...
<NoteBox type='tip'>
    **Before you read**
    ...
    This article explains how to restrict the visibility of a specific [marker](/reference/marker), [blip](/reference/blip), or [radar-area](/reference/radararea) to certain players.
    If a marker, blip, or radar area is intended to be visible to only one player, you should create it client-side instead.
    This way, it will only be visible to the local player.
</NoteBox>
...
```lua
local a = createBlip(0, 0, 0, 41, 1, 2, 3, 4, 5, 6, 9999, resourceRoot)
setElementVisibleTo(a, somePlayer, true)
```
