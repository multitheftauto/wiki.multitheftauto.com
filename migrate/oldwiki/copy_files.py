    ...
    for root, dirs, files in os.walk(source_dir):
        for file in files:
            if file.endswith(".yaml"):
                ...
                if not FORCE_COPY_ALL and os.path.exists(dest_path):
                    with open(dest_path, 'r', encoding='utf-8') as dest_file:
                        content = dest_file.read()
                        if 'requires_review: true' not in content:
                            # print(f"(YAML) Skipping {dest_path} because it does not require review")
                            continue
                ...
                with open(src_path, 'r', encoding='utf-8') as src_file:
                    with open(copy_to_dest_path, 'w', encoding='utf-8') as dest_file:
                        ...
                        func_name = file[:-5] # Remove .yaml extension
                        copied_function_names.add(func_name)
                        # print(f"(YAML) Copied {func_name} from {src_path} to {copy_to_dest_path}")
    ...
    for root, dirs, files in os.walk(source_dir):
        for file in files:
            if file.endswith(".lua"):
                ...
                extracted_func_name = file.split('-')[0].strip()
                if extracted_func_name not in copied_function_names:
