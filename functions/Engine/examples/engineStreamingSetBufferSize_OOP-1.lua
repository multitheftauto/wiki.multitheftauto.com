addCommandHandler("sbs", function(_, sizeMiB)
    local previousSizeMiB = math.floor(EngineStreaming.bufferSize / 1024 / 1024) -- Convert Bytes to MiB
    if tonumber(sizeMiB) then
        if EngineStreaming:setBufferSize(tonumber(sizeMiB) * 1024 * 1024) then -- Convert MiB to Bytes
