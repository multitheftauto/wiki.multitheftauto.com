local function outputAdminGroupAccounts(player)
    ...
    if (group) then -- Does the group exist?
        local admins = {}
        local objects = group.objects or {}
        ...
        player:outputChat(table.concat(admins, ", "), 255, 255, 255) -- output: John, Wind, Ghost
    else
        player:outputChat("The group named Admin does not exist.", player, 255, 255, 255)
    end
end
